            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-10 col-md-offset-1">
                        ...
                        <h1 class="page-heading__title">Kadrovi</h1>
            ...
            <div class="container">
                <form role="form" method="GET" class="post-filter__form clearfix forma-filteri">
                    <div class="post-filter__select">
                        <label class="post-filter__label" for="profession">Zanimanje</label>
                        <select id="profession" name="profession">
                            <option selected value="">Izaberite zanimanje</option>
                            @foreach($professions as $profession)
                                <option value="{{ $profession->id }}" {{ app('request')->input('profession') == $profession->id ? 'selected' : '' }}>{{ $profession->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="post-filter__select">
                    ...
                    <div class="post-filter__select">
                        ...
                        <select name="sort" id="sort">
                            ...
                            <option value="profession" {{ app('request')->input('sort') == 'profession' ? 'selected' : '' }}>Zanimanje</option>
                        </select>
                    </div>
                            ...
                            <div class="post-grid__item col-md-3">
                                <div class="posts__item posts__item--card posts__item--category-1 card kartica-igraca-klub">
                                    <figure class="posts__thumb club-thumb-backgr">
                                        <a href="{{url('/staff/' . $result->id)}}"><img class="logo-club-tab-index" src="{{ asset('images/staff_avatars/' . $result->image)}}" alt=""></a>
                                    </figure>
                                    <div class="posts__inner card__content">
                                        <h6 class="posts__title ime-sportiste-klub-lista"><a href="{{url('/staff/' . $result->id)}}">{{$result->firstname . ' ' . $result->lastname}}</a></h6>
                                        <div class="posts__excerpt">{{$result->city}}</div>
                                    </div>
                                    <footer class="posts__footer card__footer">
                                        <a href="{{url('/staff/' . $result->id)}}" class="btn btn-warning btn-profil-igraca"><i class="fa fa-eye"></i> Pregled profila kadra</a>
                                    </footer>
                                </div>
                            </div>
                        @endforeach
                    @else
                        <p class="text-center">Kadrovi sa definisanim parametrima ne postoje.</p>
                    @endif
                </div>
