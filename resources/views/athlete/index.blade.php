            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-10 col-md-offset-1">
                        ...
                        <h1 class="page-heading__title">Sportisti</h1>
            ...
            <div class="container">
                <form role="form" method="GET" class="post-filter__form clearfix forma-filteri">
                    <div class="post-filter__select">
                        <label class="post-filter__label" for="nature">Tip sportiste</label>
                        <select id="nature" name="nature">
                            <option selected value="">Izaberite tip sportiste</option>
                            @foreach($playerNatures as $nature)
                                <option value="{{ $nature->id }}" {{ app('request')->input('nature') == $nature->id ? 'selected' : '' }}>{{ $nature->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="post-filter__select">
            ...
            <div class="container">
                ...
                <div class="row">
                    @if($results->total() > 0)
                            ...
                            <div class="post-grid__item col-md-3">
                                <div class="posts__item posts__item--card posts__item--category-1 card kartica-igraca-klub">
                                    <figure class="posts__thumb club-thumb-backgr">
                                        <a href="{{url('/athletes/' . $result->id)}}"><img class="logo-club-tab-index" src="{{ asset('images/athlete_avatars/' . $result->avatar)}}" alt=""></a>
                                    </figure>
                                    <div class="posts__inner card__content">
                                        <h6 class="posts__title ime-sportiste-klub-lista"><a href="{{url('/athletes/' . $result->id)}}">{{$result->firstname . ' ' . $result->lastname}}</a></h6>
                        ...
                        @endforeach
                    @else
                        <p class="text-center">Sportisti sa definisanim parametrima ne postoje.</p>
                    @endif
                </div>
