						...
						<div class="post-grid__item col-md-3">
							<div class="posts__item posts__item--card posts__item--category-1 card kartica-igraca-klub">
								<figure class="posts__thumb club-thumb-backgr">
									<a href="{{url('/clubs/' . $result->id)}}"><img class="logo-club-tab-index" src="{{ asset('images/club_logo/' . $result->logo)}}" alt=""></a>
								</figure>
								<div class="posts__inner card__content">
									<h6 class="posts__title ime-sportiste-klub-lista"><a href="{{url('/clubs/' . $result->id)}}">{{$result->name}}</a></h6>
									<div class="posts__excerpt">{{$result->city}}</div>
								</div>
								<footer class="posts__footer card__footer">
									<a href="{{url('/clubs/' . $result->id)}}" class="btn btn-warning btn-profil-igraca"><i class="fa fa-eye"></i> Pregled profila kluba</a>
								</footer>
							</div>
						</div>
					@endforeach
				@else
					<p class="text-center">Klubovi sa definisanim parametrima ne postoje.</p>
				@endif
			</div>
