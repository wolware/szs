...
class TeretanaController extends Controller
{
    /**
    ...
    {
        return view('objects.teretana.new');
    }
    public function teretana_show($id){
        $teretana = DB::table('teretana')->where('id', $id)->first();
        return view('objects.teretana.profile', ['teretana' => $teretana]);
    }
    ...
    public function new(Request $data){
        ...
        if($validator->fails()){
            return redirect('objects/teretana/new')
            ...
            $id = DB::table('teretana')->insertGetId([
            ...
            if($data->hasFile('galerija')){
             ...
             return redirect('objects/teretana/'.$id);
        }
    }
